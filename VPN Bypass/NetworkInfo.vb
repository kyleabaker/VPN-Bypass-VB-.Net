...
Module NetworkInfo
    ...
    Public Sub getAdaptorInfo()
        ...
        VpnBypassImpl.log("Selected default configuration based on detected network adaptors:")
    ...
    Public Sub getPublicIpAddress()
        ...
        If isIpv4Address(response) = True Then
            publicIp = response
        End If
    ...
    Public Function getCidrFromIpCidr(ipCidr As String) As String
        ...
        If ipCidr.IndexOf("/") > -1 Then
            ' Contains CIDR
            Dim reIpv4 As Regex = New Regex("^(?:(?:(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))\/([0-9]{1,2})$")
            Dim reIpv6 As Regex = New Regex("") 'TODO

            ' Is IPv4 or IPv6?
            If reIpv4.IsMatch(ipCidr) = True Then
                Dim groups As GroupCollection = reIpv4.Match(ipCidr).Groups
                cidr = groups.Item(1).ToString
            ElseIf reIpv6.IsMatch(ipCidr) = True Then
                'TODO
            End If
        End If
    ...
    Public Function getIpAddressFromIpCidr(ipCidr As String) As String
        ...
        If isIpv4Address(ipCidr) = True Then
            ' IPv4
            ip = ipCidr
        ElseIf isIpv6Address(ipCidr) = True Then
            ' IPv6
            ip = ipCidr
    ...
    ' Check if IP Address is IPv4. Pulls ip from node and forwards to next overload.
    Public Function isIpv4Address(node As TreeNode) As Boolean
    ...
    Public Function isIpv4Address(ip As String) As Boolean
    ...
    ' Check if IP Address is IPv6. Pulls ip from node and forwards to next overload.
    Public Function isIpv6Address(node As TreeNode) As Boolean
        ' Check if node has any children (ip nodes do not have any children)
        If node.Nodes.Count = 0 Then
            Dim ip As String = node.Name
    ...
    Public Function isIpv6Address(ip As String) As Boolean
        'TODO Dim re As Regex = New Regex("")
        'Return re.IsMatch(ip)
